<script lang="ts">
  ...
  function friendlyError(err: unknown): string {
    if (err instanceof ApiError) {
      if (err.status === 409) return "That email or username is already taken. Try a different one!";
      if (err.status === 429) return "Whoa, slow down! Too many attempts. Try again in a minute.";
      if (err.message?.includes('Validation')) return "Something looks off — make sure all fields are filled in correctly.";
    }
  ...
  async function handleSubmit(e: Event) {
    ...
    if (password !== confirmPassword) {
      error = "Those passwords don't match. Give it another shot!";
      return;
    }
...
>
  ...
  <div class="flex flex-1 items-center justify-center p-6 sm:p-8">
    <div class="w-full max-w-sm">
      ...
      <h2 class="text-2xl font-extrabold tracking-tight text-text-primary mb-2">Create an account</h2>
      <p class="text-[15px] text-text-secondary mb-8">Join the conversation</p>
      ...
      <form onsubmit={handleSubmit} class="flex flex-col gap-5">
        ...
        <div class="flex flex-col gap-1.5">
          <label for="username" class="text-[11px] font-semibold text-text-muted uppercase tracking-[0.05em]">Username</label>
          <input
            id="username"
            type="text"
            bind:value={username}
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <div class="relative">
            <input
              ...
              required
              minlength={8}
              class="w-full bg-bg-tertiary border border-border rounded-[10px] px-4 py-3.5 pr-12 text-[15px] text-text-primary placeholder:text-text-muted outline-none transition-colors duration-150 focus:border-text-muted"
              placeholder="••••••••"
            />
            <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-text-muted hover:text-text-secondary transition-colors" onclick={() => showPassword = !showPassword} tabindex={-1}>
              {#if showPassword}
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>
              {:else}
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
              {/if}
            </button>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-1.5">
          <label for="confirmPassword" class="text-[11px] font-semibold text-text-muted uppercase tracking-[0.05em]">Confirm Password</label>
          <div class="relative">
            <input
              id="confirmPassword"
              type={showPassword ? 'text' : 'password'}
      ...
      <p class="text-sm text-text-secondary mt-8 text-center">
        ...
        <a href="/auth/login" class="text-accent hover:text-accent-hover transition-colors duration-150">Sign in</a>
      </p>
    </div>
  </div>
</div>
